...
const AIChatWidget: React.FC<AIChatWidgetProps> = ({ equipmentData, inventoryData }) => {
    ...
    const getModelResponse = async (promptInput: string | { toolResponses: any }) => {
        if (!chatSession.current) return;
        setIsLoading(true);
        try {
            // Correct call to Gemini API: sendMessage expects a simple object with a 'message' key if it's a prompt string
            let response = await chatSession.current.sendMessage(typeof promptInput === 'string' ? { message: promptInput } : promptInput);
        ...
        <>
            ...
            {isOpen && (
                <div 
                    ...
                    className="fixed bottom-24 right-6 w-full max-w-sm h-[500px] bg-white dark:bg-gray-800 rounded-lg shadow-2xl flex flex-col border border-gray-200 dark:border-gray-700 z-[100] animate-fade-in-up"
                >
