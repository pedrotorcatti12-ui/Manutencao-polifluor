...
export const EquipmentTypesPage: React.FC = () => {
    const { equipmentTypes, setEquipmentTypes, equipmentData, removeEquipmentType } = useDataContext();
    ...
    const handleSave = (type: EquipmentType) => {
        if (editingType) {
            ...
            const newId = type.description.trim().toUpperCase().replace(/[^A-Z0-9]+/g, '_');
            
            if (equipmentTypes.some(t => t.id === newId)) {
                alert(`Erro: Um tipo com o ID "${newId}" já existe. Tente uma descrição diferente.`);
                return;
            }
    ...
    const handleDelete = () => {
        ...
        removeEquipmentType(deletingType.id);
